...
# Cleanup container temp export dir first so deleted workflows are reflected in Git.
...
"$REPO_DIR/scripts/n8n/rename_workflows_by_name.sh" "$OUT_DIR"

# Normalize after rename (strips .id and other noisy fields).
"$REPO_DIR/scripts/n8n/normalize_workflows.sh" "$OUT_DIR"
