...
| Recipes create/search/get/update/review | `src/server/db/recipes-store.js` | `pkm.recipes` or `pkm_test.recipes` | honors active test mode | `/recipes/*` routes resolve active schema through `runtime-store` and preserve archived state semantics on recompute writes |
