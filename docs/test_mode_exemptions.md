...
| Generic PKM writes and idempotent insert/update | `src/server/db/write-store.js` | `pkm.entries` or `pkm_test.entries` by default | honors active test mode by default | `/pkm/insert*`, `/db/update`, importer writes, and ChatGPT action writes follow the persisted runtime flag unless caller explicitly chooses another table |
| Explicit Tier-1 classify writeback | `src/server/db/tier1-classify-store.js` | active schema `entries`, `active_topic_related_entries` | honors active test mode by default (or explicit schema override) | `/pkm/classify`, `/enrich/t1/update`, and batch collect writeback use explicit Tier-1 field updates plus active-topic link sync |
