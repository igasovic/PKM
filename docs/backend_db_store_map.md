...
|---|---|---|---|---|---|
| Control, debug, and ChatGPT actions | `src/server/routes/control-routes.js`, `src/server/chatgpt-actions.js`, `src/server/chatgpt/service.js` | `debug-repository`, `TestModeService`, ChatGPT service, `active-topic-repository` | `src/server/db/debug-store.js`, `src/server/db/runtime-store.js`, `src/server/db/read-store.js`, `src/server/db/write-store.js`, `src/server/db/active-topic-store.js` | `pipeline_events`, `failure_packs`, `runtime_config`, `entries`, `active_topics`, `active_topic_state`, `active_topic_open_questions`, `active_topic_action_items`, `active_topic_related_entries` | ChatGPT surface now has a dedicated first-class topic-state store alongside legacy working-memory entry reads/writes during migration |
...
| Recipes | `src/server/routes/recipes-routes.js` | `src/server/repositories/recipes-repository.js` | `src/server/db/recipes-store.js`, `src/server/db/runtime-store.js` | active schema `recipes`, `recipe_links` | recipe create/search/get/update/review/link/note endpoints use active test-mode routing |
