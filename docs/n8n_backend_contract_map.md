...
| Telegram and capture ingest flows | `POST /normalize/telegram`, `POST /ingest/telegram/url-batch`, `POST /normalize/email/intent`, `POST /normalize/email`, `POST /normalize/webpage`, `POST /normalize/notion`, `POST /db/insert`, `POST /enrich/t1`, `POST /enrich/t1/batch` | `docs/api_ingest.md`, `docs/api_read_write.md` | `test/server/classify.api-contract.test.js`, `test/server/read-write.api-contract.test.js`, `test/server/telegram-url-batch-ingest.test.js` |
