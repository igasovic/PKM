...
|---|---|---|---|
| `10 Read` (Telegram PKM command surface) | `POST /db/read/pull`, `POST /db/read/last`, `POST /db/read/continue`, `POST /db/read/find`, `POST /recipes/get`, `POST /recipes/search`, `POST /recipes/create`, `POST /recipes/link`, `POST /recipes/note`, `POST /distill/sync`, `POST /distill/run`, `GET /status/batch`, `/waiting -> 36 Todoist Waiting Radar` | `docs/api_read_write.md`, `docs/api_recipes.md`, `docs/api_distill.md`, `docs/api_todoist.md`, `docs/PRD/read-prd.md`, `docs/PRD/recipes-prd.md`, `docs/PRD/todoist-llm-planning-prd.md` | `test/server/read-write.api-contract.test.js`, `test/server/recipes.api-contract.test.js`, `test/server/tier2.api-contract.test.js`, `test/server/n8n.command-parser.test.js`, `test/server/n8n.todoist-workflows.test.js` |
...
| Telegram and capture ingest flows | `POST /normalize/telegram`, `POST /ingest/telegram/url-batch`, `POST /normalize/email/intent`, `POST /normalize/email`, `POST /normalize/webpage`, `POST /normalize/notion`, `POST /pkm/insert`, `POST /pkm/insert/batch`, `POST /enrich/t1`, `POST /enrich/t1/update`, `POST /enrich/t1/batch` | `docs/api_ingest.md`, `docs/api_read_write.md` | `test/server/classify.api-contract.test.js`, `test/server/read-write.api-contract.test.js`, `test/server/telegram-url-batch-ingest.test.js` |
