...
|---|---|---|---|
| `10 Read` (Telegram PKM command surface) | `POST /db/read/pull`, `POST /db/read/last`, `POST /db/read/continue`, `POST /db/read/find`, `POST /recipes/get`, `POST /recipes/search`, `POST /recipes/create`, `POST /distill/sync`, `POST /distill/run`, `GET /status/batch` | `docs/api_read_write.md`, `docs/api_recipes.md`, `docs/api_distill.md`, `docs/PRD/read-prd.md`, `docs/PRD/recipes-prd.md` | `test/server/read-write.api-contract.test.js`, `test/server/recipes.api-contract.test.js`, `test/server/tier2.api-contract.test.js`, `test/server/n8n.command-parser.test.js` |
