...
function routeTelegramInput(input) {
  ...
  if (s.startsWith('pkm:')) {
    return { route: 'pkm_capture', confidence: 1 };
  }
  if (s.startsWith('cal:')) {
    return { route: 'calendar_create', confidence: 1 };
  }

  const querySignal = (
    /\b(today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/.test(s)
    && /\b(what|schedule|calendar|events?|do we have|plans?)\b/.test(s)
  ) || /^\/?(status|calendar)\b/.test(s);

  const createSignal = (
    /\b(at|on|tomorrow|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/.test(s)
    && /\b(dentist|doctor|appointment|party|swim|practice|meeting|vet|trip|flight|birthday)\b/.test(s)
  ) || /\b(\d{1,2})(?::(\d{2}))?\s*(a|p|am|pm)\b/.test(s);

  if (querySignal && !createSignal) {
    return { route: 'calendar_query', confidence: 0.9 };
  }
  if (createSignal && !querySignal) {
    return { route: 'calendar_create', confidence: 0.84 };
  }
  if (!querySignal && !createSignal && s.length >= 2) {
    return { route: 'pkm_capture', confidence: 0.62 };
  }

  return {
    route: 'ambiguous',
    confidence: 0.5,
    clarification_question: 'Do you want me to add this to calendar or save it as a PKM note?',
  };
}
...
function normalizeCalendarRequest(input) {
  ...
  const timezone = text(data.timezone || config.timezone || 'America/Chicago');
  ...
  const mergedText = mergeTextWithClarificationTurns(rawText, data.clarification_turns);
  const category_code = detectCategory(mergedText, config);
  const people_codes = detectPeople(mergedText, config);
  const date_local = detectDateLocal(mergedText, timezone);
  const start_time_local = detectTimeLocal(mergedText);
  const title = deriveTitle(mergedText);
  const location = detectLocation(mergedText);
  const duration_minutes = category_code
    ? detectDurationMinutes(mergedText, category_code, config)
    : null;

  const missing = [];
  if (!title) missing.push('title');
  if (!date_local) missing.push('date');
  if (!start_time_local) missing.push('start_time');
  if (!duration_minutes) missing.push('duration');
  if (!people_codes.length) missing.push('people');
  if (!category_code) missing.push('category');
  ...
  const startPlusDuration = addMinutesWithDate(date_local, start_time_local, duration_minutes);
  const end_date_local = startPlusDuration ? startPlusDuration.date_local : date_local;
  const end_time_local = startPlusDuration ? startPlusDuration.time_local : start_time_local;
  const isHome = location && (config.padding.home_literals || []).some((v) => lower(v) === lower(location));
  ...
  const blockStart = addMinutesWithDate(date_local, start_time_local, -padBefore);
  const blockEnd = addMinutesWithDate(end_date_local, end_time_local, padAfter);

  const peopleTag = resolveSubjectPeopleTag(people_codes, config);
  const subjectCode = `[${peopleTag}][${category_code}] ${formatClock12(start_time_local)} ${title}`;
  const color = resolveColor(people_codes, config);
  ...
  return {
    ...
    normalized_event: {
      ...
      original_start: {
        date_local,
        time_local: start_time_local,
      },
      block_window: {
        start_date_local: blockStart ? blockStart.date_local : date_local,
        start_time_local: blockStart ? blockStart.time_local : start_time_local,
