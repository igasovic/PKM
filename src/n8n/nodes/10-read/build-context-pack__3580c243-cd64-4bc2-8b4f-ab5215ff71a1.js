...
function loadContextPackBuilder() {
  const packagedPath = path.join(__dirname, '..', '..', 'shared', 'context-pack-builder.js');
  try {
    return require(packagedPath);
  } catch (_err) {
    return require('@igasovic/n8n-blocks/shared/context-pack-builder.js');
  }
}
...
const { buildContextPackMarkdown } = loadContextPackBuilder();
const { mdv2Render } = loadTelegramMarkdown();

module.exports = async function run(ctx) {
  const rows = ctx.$input.all().map((item) => item.json);
